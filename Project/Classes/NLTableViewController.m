...
@interface NLTableViewController () <UITableViewDataSource, UITableViewDelegate>
...
- (void)tableViewDidPullToRefresh:(UIRefreshControl *)control {
    [self.automatic getTrips:^(NSError *error, id responseObject) {
        ...
        [responseObject enumerateObjectsUsingBlock:^(NLAutomaticTrip *trip, NSUInteger idx, BOOL *stop) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    NLAutomaticTrip *trip = self.tripDict[title][@"trips"][indexPath.row];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    NLMapViewController *mapVC = [NLMapViewController new];
    NSString *title = self.sectionKeys[indexPath.section];
